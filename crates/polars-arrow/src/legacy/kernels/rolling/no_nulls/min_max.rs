...
//     ($m_window_bool:tt, $value:ident) => {
//         pub struct $m_window_bool {}

//         impl<'a, Fo: Fn(Idx, WindowSize, Len) -> (Start, End)> RollingAggWindowBoolNoNulls<Fo>
//             for $m_window_bool
//         {
//             fn result_values(
//                 bitmap: &Bitmap,
...
//                 // Set default to true
//                 out.extend_constant(len, !($value));

//                 let value_bitmap = match $value {
//                     true => bitmap,
//                     false => &bitmap.not(),
//                 };
...
//                 let mut last_end = 0;
//                 // loop all false idx
//                 for value_idx in value_bitmap.true_idx_iter() {
//                     // window that should be set to false
//                     let (start, end) = det_effects_fn(value_idx, window_size, len);
...
// minmax_window_bool!(MinWindowBool, false);
// minmax_window_bool!(MaxWindowBool, true);
    ...
    ($m_window_bool:tt, $count_until_next_value_fn:path, $count_consec_value_fn:path, $value:ident) => {
            ...
            for $m_window_bool
        {
            fn result_values(
                ...
                while idx < len {
                    // window that should be unset
                    let (start, end) = det_effects_fn(idx, window_size, len);

                    // extend window if there's more
                    let zeros = $count_consec_value_fn(iter);
                    let end = std::cmp::min(len, end + (zeros - 1));

                    // previous false not in current window
                    if last_end <= start {
                        // Update value before current window
                        out.extend_constant(start - last_end, !($value));
                        // Update all value in window
                        out.extend_constant(end - start, $value);
                    }
                    // previous false in current window
                    else {
                        // Update new value in window
                        out.extend_constant(end - last_end, $value);
                    }
                    last_end = end;

                    // count until the next false
                    idx += zeros + $count_until_next_value_fn(iter);
                }
...
//             // extend window if there's more
//             let zeros = iter.take_leading_zeros();
//             let end = std::cmp::min(len, end + (zeros - 1));

//             // previous false not in current window
//             if last_end <= start {
//                 // Update value before current window
//                 out.extend_constant(start - last_end, true);
//                 // Update all value in window
//                 out.extend_constant(end - start, false);
//             }
//             // previous false in current window
//             else {
//                 // Update new value in window
//                 out.extend_constant(end - last_end, false);
//             }
//             last_end = end;

//             // count until the next false
//             idx += zeros + iter.take_leading_ones();
//         }

//         // fill the remaining
//         out.extend_constant(len - last_end, true);
...
//         idx += BitmapIter::take_leading_ones(iter);

//         // Count until the first true
//         idx += iter.take_leading_zeros();

//         while idx < len {
//             // window that should be set
//             let (start, end) = det_effects_fn(idx, window_size, len);

//             // extend window if there's more
//             let ones = iter.take_leading_ones();
//             let end = std::cmp::min(len, end + (ones - 1));

//             // update value
//             if last_end <= start {
//                 // previous true not in current window

//                 // Update value before current window
//                 out.extend_constant(start - last_end, false);
//                 // Update all value in window
//                 out.extend_constant(end - start, true);
//             } else {
//                 // Update new value in window
//                 out.extend_constant(end - last_end, true);
//             }
//             last_end = end;

//             // count until the next true
//             idx += ones + iter.take_leading_zeros();
//         }

//         // fill the remaining
//         out.extend_constant(len - last_end, false);
...
// weights are handled by rolling_min/rolling_max
    ...
    ($rolling_m:ident, $window:tt) => {
        pub fn $rolling_m(
            ...
            let effect_fn = match center {
                true => det_offsets_center,
                false => |i, window_size, len| (i, std::cmp::min(len, i + window_size)),
            };
